...
import PastReportsBody from '../components/pastReportsComponents/PastReportsBody'
...
export default function PastReports({ userInfo }) {
            ...
            <Container maxWidth="xl">
              <div className={styles.profilePageBodyDiv}>
                  <div className={styles.profileNavigation}>
                    ...
                    <div className={styles.sideNavElement}>
                      <Image src={reportIcon} alt="dashboard icon" />
                      <span>Past Reports</span>
                    </div>
                    <div style={{marginBottom: '0'}} className={styles.navigationDivider}></div>
                  </div>
                  <PastReportsBody user={userInfo} />
              </div>
          </Container>
        </div>
    )
...
export async function getServerSideProps(context) {
    ...
    if (data.accessToken) {
      context.res.setHeader('Set-Cookie', ['access-token=' + data.accessToken + '; Max-Age=1200; HttpOnly'])
    }
